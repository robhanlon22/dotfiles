{
  ...
  options.my.programs.yabai.enable = lib.mkEnableOption "yabai";

  config = lib.mkIf config.my.programs.yabai.enable {
    assertions = [
      {
        assertion = pkgs.stdenv.isDarwin;
        message = "yabai is Darwin-only";
      }
    ];
    ...
    launchd.agents = my.darwin.launchdAgents {
      ...
      in ["${yabai}" "--verbose" "--config" "${yabairc}"];
    };
  };
}
